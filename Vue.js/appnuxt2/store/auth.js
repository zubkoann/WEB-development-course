      ...
      this.$axios.$post('/register', authData)
        .then((data) => {
            if (data.item) {
              ...
              // если обращаемся к мутации в этом же модуле
              commit('setUserData', data.item);
              // если обращаемся к мутации в другом  модуле сначала название потом мутацию

              commit('notifications/setSuccessMessage', message, {
                root: true
              });
              this.$router.push('/cabinet');
              commit('notifications/setNotificationStatus', true, {
                root: true
              });
            }
            // resolve(data);
